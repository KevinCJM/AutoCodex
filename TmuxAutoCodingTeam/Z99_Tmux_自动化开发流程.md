1. **AGENT初始化阶段** 
   ...
   - 由 [人类] 指定, 是否需要执行 AGENTS init 程序. 该程序用于生成项目路由层.
     - 指定执行位置, 可以在多个文件夹下执行, 并行执行
       - 例如: 在 tmux-api-v3/tmux-api-v3 下执行, 在 tmux-api-v3/tmux_api_v3/core/calculation 下执行
...
5. **详细设计阶段** 
   ...
   - 如果是直接评审现有详细设计:
     - 创建各个不同的评审角色: 人类可以设置 <角色定位> 的智能体: 开发工程师, 测试工程师, 审核员, 等等 (有默认值). 并且可以给不同的 <角色定位> 设置角色定位提示词 (有默认值). 智能体名称仍然使用 "{角色定位}-{角色名称}" 的逻辑. 并且为各个智能体选择厂商/模型/推理
       - 并行让各个智能体审核`{需求名}_详细设计.md`文档, 用提示词 `Prompt_05_DetailedDesign.review_detailed_design`
       - 各个智能体将评审结果写入各自的 `{需求名}_详设评审记录_{智能体名称}.md` 中, 再统一合并到 `{需求名}_详设评审记录.md` 中
         - 用 check_reviewer_job 函数检查智能体有没有按提示词要求更新, 如果没有则触发提醒提示词, 要求智能体更新文档
         - 用 task_done 函数判断是否所有评审都通过: 1)合并所有md, 2)判断总md是否为空, 3)判断所有json是否true
     - 如果 全部评审通过, 则进入 <任务拆分阶段>
     - 如果 评审未通过
       - 让用户创建 [需求分析师] 智能体, 选择厂商/模型/推理
       - 先使用提示词 `Prompt_05_DetailedDesign.create_detailed_design_ba` 初始化
       - 然后读取 `{需求名}_详设评审记录.md` 评审建议, 要求 [需求分析师] 根据评审记录对 `{需求名}_详细设计.md` 进行优化, 并解答疑问与歧义. 用 `Prompt_05_DetailedDesign.modify_detailed_design` 提示词
         - 如果 [需求分析师] 发起 HITL (`{需求名}_与人类交流.md`不为空), 将人类的反馈发给 [需求分析师] 用 `Prompt_05_DetailedDesign.hitl_relpy` 提示词. 
         - 如果 [需求分析师] 没有发起 HITL (`{需求名}_与人类交流.md`为空), 那么读取 `{需求名}_需求分析师反馈.md` 用 `Prompt_05_DetailedDesign.again_review_detailed_design` 提示词发给各个评审智能体.
       - 各个审核智能体再次审核, 直到全部回复 "审核通过", 则进入 <任务拆分阶段>
...
6. **任务拆分阶段**
   ...
   - 如果是直接评审现有任务单:
     - 创建各个不同的评审角色: 人类可以设置 <角色定位> 的智能体: 开发工程师, 测试工程师, 审核员, 等等 (有默认值). 并且可以给不同的 <角色定位> 设置角色定位提示词 (有默认值). 智能体名称仍然使用 "{角色定位}-{角色名称}" 的逻辑. 并且为各个智能体选择厂商/模型/推理
       - 先并行初始化各个审核智能体, 用 用提示词 `Prompt_06_TaskSplit.create_task_split_ba`, 
       - 然后并行让各个智能体审核`{需求名}_任务单.md`文档, 用提示词 `Prompt_06_TaskSplit.review_detailed_design`
       - 各个智能体将评审结果写入各自的 `{需求名}_任务单评审记录_{智能体名称}.md` 中, 再统一合并到 `{需求名}_任务单评审记录.md` 中
         - 用 check_reviewer_job 函数检查智能体有没有按提示词要求更新, 如果没有则触发提醒提示词, 要求评审智能体更新文档
         - 用 task_done 函数判断是否所有评审都通过: 1)合并所有md, 2)判断总md是否为空, 3)判断所有json是否true
     - 如果 全部评审通过, 则进入 <任务开发阶段>
     - 如果 评审未通过
       - 让用户创建 [需求分析师] 智能体, 选择厂商/模型/推理
       - 先使用提示词 `Prompt_06_TaskSplit.create_task_split_ba` 初始化
       - 然后读取 `{需求名}_任务单评审记录.md` 评审建议, 要求 [需求分析师] 根据评审记录对 `{需求名}_任务单.md` 进行优化, 并解答疑问与歧义. 用 `Prompt_06_TaskSplit.modify_task` 提示词
         - 再次调用各个评审角色进行评审, 用 `Prompt_06_TaskSplit.again_review_task` 然后循环直到全部评审通过
...
7. **任务开发阶段**
   ...
   - 创建 [开发工程师] 智能体, 可以指定角色定位提示词, 也可以用系统默认的角色定位提示词
     - 初始化 [开发工程师] 智能体, 使用 `Prompt_07_Development.init_developer` 做初始化
       - [开发工程师] 智能体认为 '准备就绪': `{需求名}_与人类交流.md` 为空
       - [开发工程师] 智能体认为 '阻断': 写 `{需求名}_与人类交流.md` 触发 HITL
         - 人类的回复使用 `Prompt_07_Development.human_reply` 回馈给 [开发工程师] 智能体
           - [开发工程师] 智能体认为 '准备就绪': `{需求名}_与人类交流.md` 为空
           - [开发工程师] 智能体认为 '阻断': 写 `{需求名}_与人类交流.md` 触发 HITL. 循环直到 [开发工程师] 智能体认为 '准备就绪' 并且 `{需求名}_与人类交流.md` 为空
   - 创建各个不同的评审角色
   ...
   - [开发工程师] 智能体认为 '准备就绪' 后, 让人类选择开发工程师可用的subagent数量. 然后使用 get_first_false_task 函数判断下一个需要开发的任务. 然后使用 `Prompt_07_Development.start_develop` 进行开发
     ...
     - 如果全部评审未通过
       ...
       - 循环直到全部任务开发完成
   - 全部任务开发完成之后, 要求 [开发工程师] 智能体以及各个不同的评审角色执行
   - 需要支持断点重新开始: 可能之前已经开发了部分任务, 需要支持重新创建 [开发工程师] 智能体和各个不同的评审角色, 从下一个未完成的任务开始开发流程.
   - 需要支持根据对话轮数重新创建 [开发工程师] 智能体和各个不同的评审角色, 从下一个未完成的任务开始开发流程. 这是为了避免智能体上下文膨胀腐化.
     - 用户可以指定 [开发工程师] 智能体的最大对话轮数, 可以选择无限轮 (不重建), 默认使用15轮对话. 超过15轮对话后就要在本任务结束后下个任务开始前重新创建 [开发工程师] 智能体和各个不同的评审角色. 

8. **复核阶段**
   - 进入复核阶段前, 必须确保 `{需求名}_任务单.json` 和 `{需求名}_任务单.md` 存在. 并且 `{需求名}_任务单.json` 下的所有任务被标注为 true
   - 延用 任务开发阶段 的主工作智能体以及 审核智能体
   - 如果没有经历 任务开发阶段 跳入到 复核阶段 的, 需要让用户创建 [开发工程师] 智能体, 以及创建各个不同的评审智能体
     - 人类可以给 [开发工程师] 智能体选择厂商/模型/推理强度/代理端口
     - 人类可以设置 <角色定位> 的评审智能体: 需求分析师, 测试工程师, 审核员, 架构师, 等等 (有默认值). 并选择各个审核智能体的厂商/模型/推理强度/代理端口
   - 复核阶段流程
     - 初始化评审智能体, 用 `Prompt_08_OverallReview.init_reviewer` 提示词
     - 让评审智能体基于需求和详细设计审核整体代码, 用 `Prompt_08_OverallReview.review_all_code` 提示词
       - 审核完成后检查评审智能体有没有正常写文档, 用 check_reviewer_job 函数. 如果有评审智能体没有正常写文档则发送提醒提示词. 然后再次检查.
       - 用 task_done 函数, 判断是否审核通过
     - 如果审核通过则进入下一阶段
     - 如果审核未通过
       - 初始化 [开发工程师] 智能体, 用 `Prompt_08_OverallReview.init_reviewer` 提示词
       - 用 get_markdown_content 函数读取评审建议, 让开发工程师改代码用 `Prompt_08_OverallReview.refine_all_code` 提示词. 
       - 完成后检查 [开发工程师] 智能体有没有写 `name_工程师开发内容.md`, 如果没有则发提醒提示词
       - 用 get_markdown_content 函数读取工程师开发内容, 让评审智能体再次评审用 `Prompt_08_OverallReview.review_all_code_again` 提示词.
       - 循环直到全部评审通过
