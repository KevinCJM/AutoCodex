...
from T02_tmux_agents import AgentRuntimeInterventionRequired, AgentStartupInterventionRequired
...
class _FakeWorker:
    ...
    def run_turn(
        ...
        *,
        ...
        completion_contract,
        timeout_sec,
        runtime_intervention_handler=None,
    ):  # noqa: ANN001
        _ = runtime_intervention_handler
        self.turn_calls += 1
...
class HitlRuntimeTests(unittest.TestCase):
    ...
    def test_run_hitl_agent_loop_handles_runtime_intervention_inside_same_turn(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            output_path = root / "output.md"
            question_path = root / "question.md"
            ...
            def behavior(worker, label, prompt, completion_contract, timeout_sec):  # noqa: ANN001
                _ = worker, label, prompt, timeout_sec
                output_path.write_text("最终正文\n", encoding="utf-8")
                _write_stage_status(
                    stage_status_path,
                    stage="demo_stage",
            ...
            class RuntimeInterventionWorker(_FakeWorker):
                def __init__(self):
                    super().__init__(behavior, runtime_dir=root / "runtime")
                    self.ensure_attempts = 0
                    self.ensure_error = AgentRuntimeInterventionRequired(
                        blocker_kind="opencode_permission",
                        session_name="opencode-hitl-session",
                        state_path=str(root / "runtime" / "worker.state.json"),
                        message="Agent runtime requires manual intervention before turn",
                    )
                ...
                def run_turn(self, *, runtime_intervention_handler=None, **kwargs):  # noqa: ANN003
                    self.turn_calls += 1
                    if not callable(runtime_intervention_handler):
                        raise self.runtime_error
                    runtime_intervention_handler(self, self.runtime_error)
                    self.behavior(
                        self,
                        kwargs["label"],
                        kwargs["prompt"],
                        kwargs["completion_contract"],
                        kwargs["timeout_sec"],
                    )
                    kwargs["completion_contract"].validator(kwargs["completion_contract"].status_path)
                    return type("CommandResult", (), {"ok": True, "clean_output": "", "exit_code": 0})()

            worker = RuntimeInterventionWorker()
            handled: list[tuple[object, AgentRuntimeInterventionRequired]] = []
            result = run_hitl_agent_loop(
                worker=worker,
                stage_name="demo_stage",
                output_path=output_path,
        ...
        self.assertEqual(worker.ensure_attempts, 2)
        self.assertEqual(worker.turn_calls, 1)
        self.assertEqual(handled, [(worker, worker.ensure_error), (worker, worker.runtime_error)])
