...
# 详细设计模式下的需求说明
requirement_str = """
当前代码无法解析有多级标题架构时的word文档. 不管模板里有多少级标题和正文，最终都会只输出 一个标题样式 和 正文样式.
我需要“复杂模板识别”, 能识别不同层级标题 + 对应层级正文, 以及 摘要标题, 图表标题, 参考文献标题 等特殊标题与下属文字的样式.
"""
# 详细设计模式下各个智能体初始化提示
...
将详细设计写入 {design_md} 中.
""",
    '审核员': f"""现在起, 你是一个专业的代码与架构审核员. $System Architect
现在有以下需求补充:
```
{requirement_str}
```
...
返回你发现的错误/遗漏点 以及 疑问/歧义点.
不要返回多余的信息. 不要修改代码与文档.
""",
    '测试工程师': f"""现在起, 你是一个专业的测试工程师. $Business Analyst
现在有以下需求补充:
```
{requirement_str}
```
...
不要返回多余的信息. 不要修改代码与文档.
""",
    '开发工程师': f"""现在起, 你是一个专业的Python开发工程师. $Developer
现在有以下需求补充:
```
{requirement_str}
```
