...
def _worker_selection_from_state(state: dict[str, object]) -> ReviewAgentSelection | None:
    ...
    return ReviewAgentSelection(
        ...
        codegraph_mode=str(config.get("codegraph_mode", "") or "off").strip() or "off",
        codegraph_config=(
            dict(config.get("codegraph_config", {}))
            if isinstance(config.get("codegraph_config", {}), dict)
            else {}
        ),
    )
    ...
    *,
    ...
    for state_path, state in _iter_scoped_development_worker_states(project_dir, requirement_name):
        ...
        if has_expected_policy and (
            ...
            or str(selection.codegraph_mode or "off").strip()
            != str(expected_selection.codegraph_mode or "off").strip()
            or normalize_codegraph_config(selection.codegraph_config)
            != normalize_codegraph_config(expected_selection.codegraph_config)
        ):
            continue
    ...
    *,
    ...
    def _developer_output_fallback_payload() -> dict[str, object] | None:
        ...
        if result_contract.mode not in {"a07_developer_task_complete", "a07_developer_refine"}:
            return None
        _, _, task_status_path, _ = _current_task_contract_paths()
        if task_status_path is not None:
            try:
                output_mtime_ns = paths["developer_output_path"].stat().st_mtime_ns
                task_started_mtime_ns = task_status_path.stat().st_mtime_ns
            except OSError:
                return None
            if output_mtime_ns < task_started_mtime_ns:
                return None
        code_change = get_markdown_content(paths["developer_output_path"]).strip()
        if not code_change:
            return None
    ...
    while True:
        try:
            payload = run_task_result_turn_with_repair(
                ...
                task_name=task_name,
                codegraph_context=CodeGraphTurnContext(
                    stage_key="A07",
...
def collect_reviewer_agent_selections(
    *,
    ...
    default_codegraph_config: dict[str, object] | None = None,
    ...
    for reviewer_spec in reviewer_specs:
        ...
        if interactive:
            ...
            selection = replace(
                selection,
                codegraph_mode=default_codegraph_mode,
                codegraph_config=dict(default_codegraph_config or {}),
            )
...
def _run_parallel_reviewers(
    ...
    *,
    ...
    if change_ledger_path is not None:
        change_set = record_source_task_changes(
            change_ledger_path,
            project_dir=project_dir,
            task_name=task_name,
            source_config=None,
    ...
    *,
    ...
    try:
        ...
        change_ledger = (
            ...
            if agent_config.codegraph_mode != CodeGraphMode.OFF.value
            else None
        )
                ...
                if not review_round_policy.initial_review_done:
                    ...
                    reviewer_workers, developer = run_reviewer_phase_with_death_handling(
                        ...
                        run_phase=lambda active_reviewers: _run_parallel_reviewers(
                            ...
                            stage_runner_id=source_stage_runner_id,
                        ),
                        replace_dead_main_owner=replace_dead_developer_owner,
