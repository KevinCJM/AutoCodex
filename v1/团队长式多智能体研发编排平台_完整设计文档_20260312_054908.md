...
**会话级代理（Session Proxy）**
指在某个 tmux session / window 启动智能体 CLI 前，先注入一组网络环境变量，使该成员会话走指定代理端口。  
典型场景是访问模型服务、文档站点或第三方依赖时需要 VPN / 本地代理。  
这类代理配置应被视为成员运行时配置的一部分，允许按成员默认配置，也允许按本轮运行计划临时覆盖。
...
- 某个成员本轮临时改用哪种 CLI backend、模型 / profile 或代理端口
...
| `member_runtime_overrides` | 本轮临时覆盖成员的 CLI backend、模型 / profile、代理和 session_env 等运行时参数 |
...
1. 启动前环境注入  
   包括 `session_env`、代理变量、其他 prelaunch 命令。
...
- 注册所有成员实例
- 解析成员配置
- 维护 session name / worktree / writable paths / runtime_backend / proxy
...
- 注入角色 prompt
- 注入任务驱动 prompt
- 合成 proxy / session_env / prelaunch 命令
...
## 27.3 为什么 artifacts 和 inbox/outbox 要分开？
因为正式工件与运行时消息语义不同。  
混在一起后期会很难治理。
...
| VPN / 代理配置漂移 | 某些成员会话无法访问模型或外网资源 | 将代理端口纳入成员运行时配置，并在 tmux 启动前自动注入环境变量 |
...
16. **会话级代理应作为正式运行时能力，允许在 tmux 启动 CLI 前自动注入指定端口的代理环境变量。**
