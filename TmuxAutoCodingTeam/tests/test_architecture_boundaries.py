...
class ArchitectureBoundaryTests(unittest.TestCase):
    def test_top_level_entrypoints_are_real_module_aliases(self):
        ...
        from tmux_core.bridge import backend
        from tmux_core.runtime import contracts, hitl, tmux_runtime
        from tmux_core.stage_kernel import detailed_design, overall_review, requirements_review
        from tmux_core.workflow import entry
    ...
    def test_prompt_contract_modules_do_not_import_runtime_or_stage_kernel(self):
        for relative_path in (
            "tmux_core/prompt_contracts/spec.py",
            "tmux_core/prompt_contracts/common.py",
            "tmux_core/prompt_contracts/requirements_clarification.py",
            "tmux_core/prompt_contracts/requirements_review.py",
            "tmux_core/prompt_contracts/detailed_design.py",
            "tmux_core/prompt_contracts/task_split.py",
            "tmux_core/prompt_contracts/overall_review.py",
        ):
            imports = _collect_imports(relative_path)
            self.assertFalse(
                any(name.startswith("tmux_core.runtime") for name in imports),
                relative_path,
            )
            self.assertFalse(
                any(name.startswith("tmux_core.stage_kernel") for name in imports),
                relative_path,
            )

    def test_internal_workflow_bridge_and_runtime_use_package_modules(self):
        workflow_imports = _collect_imports("tmux_core/workflow/entry.py")
        bridge_imports = _collect_imports("tmux_core/bridge/backend.py")
        hitl_imports = _collect_imports("tmux_core/runtime/hitl.py")
        self.assertIn("tmux_core.stage_kernel.requirements_review", workflow_imports)
        self.assertIn("tmux_core.stage_kernel.detailed_design", workflow_imports)
        self.assertIn("tmux_core.stage_kernel.overall_review", workflow_imports)
        self.assertIn("tmux_core.runtime.tmux_runtime", workflow_imports)
        ...
        self.assertIn("tmux_core.workflow.entry", bridge_imports)
        self.assertIn("tmux_core.stage_kernel.requirements_review", bridge_imports)
        self.assertIn("tmux_core.stage_kernel.detailed_design", bridge_imports)
        self.assertIn("tmux_core.stage_kernel.overall_review", bridge_imports)
        self.assertIn("tmux_core.runtime.tmux_runtime", bridge_imports)
        ...
        self.assertIn("tmux_core.runtime.contracts", hitl_imports)
        self.assertIn("tmux_core.runtime.tmux_runtime", hitl_imports)
        self.assertNotIn("T02_tmux_agents", hitl_imports)
        contracts_imports = _collect_imports("tmux_core/runtime/contracts.py")
        self.assertNotIn("T04_common_prompt", contracts_imports)

    def test_agent_runtime_state_has_single_source_of_truth(self):
        runtime_source = (PROJECT_ROOT / "tmux_core/runtime/tmux_runtime.py").read_text(encoding="utf-8")
        ...
        bridge_source = (PROJECT_ROOT / "tmux_core/bridge/backend.py").read_text(encoding="utf-8")
        ...
        stage_source = (PROJECT_ROOT / "tmux_core/stage_kernel/role_orchestration.py").read_text(encoding="utf-8")
