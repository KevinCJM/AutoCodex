...
def human_feed_bck(human_msg, *, original_requirement_md='name_原始需求.md', ask_human_md='name_与人类交流.md',
...
## 处理流水线 (Silent SOP)
1. **语义解构**：从下方数据块中提取 [Q] 提问、[A] 建议、[N] 新信息。
2. **合规审计**：依据《{original_requirement_md}》执行“最小改动”原则。严禁非必要的功能蔓延，阻断所有未经授权的 Scope Creep。
3. **物理同步**：
    - 更新《{requirements_clear_md}》：合并合理变更，保持输入/规则/输出/异常的闭环。
    - 更新《{hitl_record_md}》：极简标签格式，物理删除冲突旧规，维护单一真理来源。
    - 写入《{ask_human_md}》：**在此文档中**完整回答提问，并详细列出本次【修改清单】与【拦截清单】。

## 写入协议
- 《{ask_human_md}》是本次交互的唯一信息输出地。
- 包含：1. 对人类提问的直接回答；2. 澄清文档的修改摘要；3. 被拦截的需求风险警告。

## 响应限制
- **严禁**在当前对话框输出分析过程、执行摘要或任何自然语言。
- **唯一合法返回**：
```text
完成
```
