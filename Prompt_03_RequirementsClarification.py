# -*- encoding: utf-8 -*-
"""
@File: Prompt_03_RequirementsClarification.py
@Modify Time: 2026/4/17
@Author: Kevin-Chen
@Descriptions: 需求澄清阶段提示词
"""
...
* **实事求是**: 不要过度设计,只针对核心本质的目标。不要过度假设,只根据代码事实做分析。
...
当且仅当所有逻辑分支清晰，且解答完所有疑问后。将需求澄清写入文档: 《{requirements_clear_md}》
然后直接返回字符串：
```text
信息足够
```
...
## Context & Scope
你负责对《{original_requirement_md}》进行代码级的需求拆解。你必须像审计员一样审视代码与需求之间的裂痕。可以参考《{hitl_record_md}》如果该文档存在。
