...
export function normalizeCodeGraphStatus(value: unknown): CodeGraphStatus | undefined {
  ...
  return {
    ...
    pendingChanges: { added: count(pending.added), modified: count(pending.modified), removed: count(pending.removed) },
...
export function codegraphStatusTone(status: CodeGraphStatus | undefined): string {
  if (!status) return 'muted'
  if (status.state === 'ready') return 'good'
  if (['initializing', 'syncing', 'stale', 'degraded'].includes(status.state)) return 'active'
