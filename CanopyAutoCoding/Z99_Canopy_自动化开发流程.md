...
2. **需求分析阶段** 基于需求说明文档分析要做什么
   - 需求录入
     - 要求用户输入需求名称
     - 通过 文档/文字输入/Notion链接 等方式, 将原始需求写入 `{需求名}_原始需求.md`
       - 如果是 Notion 则启动一个 codex, GPT-5.4-Mini, high 临时智能体用 $notion-api-token-ops 这个技能读取需求. 提示词用 Prompt_02.get_notion_requirement
   - 需求分析:
     1. [需求分析师] 智能体通过了解代码, 以及需求说明, 分析需求. 使用提示词 Prompt_02.requirements_understand
     2. 如果 [需求分析师] 智能体认为信息不足以澄清需求, 则要求HITL人类提供更多信息. 将提问写入`{需求名}_与人类交流.md`, 由 [系统] 提交给人类
        2.1 将人类的答复返回给 [需求分析师] 智能体. 使用提示词 `Prompt_02_RequirementsAnalysis.hitl_bck`
...
3. **需求澄清评审阶段**
     - 人类审核 `需求澄清.md`
      1. 如果审核通过, 则进入 需求评审阶段
      2. 如果审核未通过, 则人类发出提问. 使用提示词 `Prompt_02_RequirementsAnalysis.human_feed_bck`
        - [需求分析师] 智能体基于人类反馈, 修改 `{需求名}_需求澄清.md` 或者向人类回复解释
      3. 重复, 直到人类审核通过, 进入 需求评审阶段
    - 审核智能体审核:
      1. 启动N个 [审核器] 智能体, 根据 `{需求名}_原始需求.md` + `{需求名}_人机交互澄清记录.md` 评审 `{需求名}_需求澄清.md` 文档, 每个 [审核器] 要有编号, 并行执行. 使用提示词 `Prompt_02_RequirementsAnalysis.requirements_understand`
      ...
      3. 评审若未通过, 则将审核结果交予 [需求分析师] 优化和修改. 使用提示词 `Prompt_02_RequirementsAnalysis.review_feedback`
         3.1 如果有 [需求分析师] 认为无法决定的, 或者信息不足时, 触发HITL. 流程回到 需求分析阶段 的 (2) 
         3.2 如果信息足够, [需求分析师] 回复 [审核器] 智能体再次审核. 使用提示词 `Prompt_02_RequirementsAnalysis.requirements_review_reply`
         3.3 重复, 直到所有 [审核器] 智能体判断审核通过, 然后可以进入 详细设计阶段

4. **详细设计阶段** 
   - [需求分析师] 分析完需求拿到所有必要信息并通过评审后, 写详细设计文档. 提示词见 Prompt_03_DetailedDesign.detailed_design
   - 设置各个不同的评审角色, 基于需求与澄清文档评审 `详细设计.md`, 并行执行.
     - 人类可以设置各种背景的智能体: 开发工程师, 测试工程师, 审核员, 等等.
     - 用评审提示词 Prompt_03_DetailedDesign.review_detailed_design
     - 各个智能体将评审结果写入各自的 `详设评审记录_{智能体名称}.md` 中, 再统一合并到 `详设评审记录.md` 中
     - [需求分析师] 根据 `详设评审记录.md` 对 `详细设计.md` 进行优化, 并解答疑问与歧义
     - 各个智能体再次审核, 直到全部回复 "审核通过"
   - 换一个评审提示词: Prompt_03_DetailedDesign.second_review_detailed_design, 重新要求各个智能体进行评审, 直到全部回复 "审核通过"
...
5. **任务拆分阶段**
   ...
   - 各个智能体再次审核, 直到全部回复 "审核通过"
   - [需求分析师] 基于 `任务单.md` 输出 `任务单.json`. 用提示词 Prompt_05.task_md_to_json
