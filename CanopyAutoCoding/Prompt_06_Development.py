...
### 第三阶段：状态持久化 (Sync)
- **任务同步**：在《{task_split_md}》中将任务项 `{task_title}` 标记为已完成 `[x]`。
- **里程碑检查**：若当前里程碑下所有任务均已勾选，必须同步更新里程碑状态为“已完成”。
...
本轮为**二次审核**。你需要判定开发工程师是否已根据上一轮审计建议完成了逻辑闭环。
...
3. **回归校验 (Regression Check)**：重点审计修复动作是否意外破坏了原本稳定的逻辑，或引入了非必要的复杂度。
...
if __name__ == '__main__':
    ...
    '''开发完成后做审核'''
    code_change_msg = get_markdown_content(
        '/Users/chenjunming/Desktop/Canopy/canopy-api-v3/RealLifeCost_工程师开发内容.md')
    ...
    pass_bool = task_done(directory="/Users/chenjunming/Desktop/Canopy/canopy-api-v3",
                          file_path='/Users/chenjunming/Desktop/Canopy/canopy-api-v3/RealLifeCost_任务单.json',
                          task_name=task,
                          json_pattern=f"{requirement_name}_评审记录_*.json",
                          md_pattern=f"{requirement_name}_代码评审记录_*.md",
    ...
    if pass_bool:
        ...
        c_msg = get_markdown_content('/Users/chenjunming/Desktop/Canopy/canopy-api-v3/RealLifeCost_代码评审记录.md')
        # print(refine_code(c_msg, task, detailed_design_md=f'{requirement_name}_详细设计.md',
        #                   what_just_dev=f'{requirement_name}_工程师开发内容.md'))
        c_change = get_markdown_content('/Users/chenjunming/Desktop/Canopy/canopy-api-v3/RealLifeCost_工程师开发内容.md')
        # print(re_review_code(task, c_msg, c_change,
        #                      task_split_md=f'{requirement_name}_任务单.md',
        #                      detailed_design_md=f'{requirement_name}_详细设计.md',
        #                      review_md=f'{requirement_name}_代码评审记录_G.md',
        #                      review_json=f'{requirement_name}_评审记录_G.json'
        #                      ))
